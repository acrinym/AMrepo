...
class ThresholdConsolidator:
    ...
    def is_block_start(self, line: str) -> bool:
        ...
        # Check for various block start patterns - EXPANDED to catch all formats
        patterns = [
            # Standard markdown headers
            r'^#{1,3}\s*(?:Threshold|Entry|AmandaMap|Phoenix Codex)',
            r'^#{1,3}\s*AmandaMap\s+(?:Entry|Threshold)',
            r'^#{1,3}\s*Phoenix\s+Codex\s+(?:Entry|Threshold)',
            ...
            # Additional AmandaMap patterns found in actual chats
            r'AmandaMap\s+Threshold\s+\d+[:\-–—]',  # Threshold 38:, Threshold 27 –, etc.
            r'AmandaMap\s+Threshold\s+\d+\s*\(',     # Threshold 54 (In Progress)
            r'AmandaMap\s+Threshold\s+\d+\s*"',      # Threshold 53: "Title"
            r'AmandaMap\s+Threshold\s+\d+\s*\*',     # Threshold 28 – *Title*
            
            # Phoenix Codex patterns
            r'Phoenix\s+Codex\s+Threshold\s+\d+',
            r'Phoenix\s+Codex\s+Entry\s+\d+',
            
            # General threshold patterns
            r'^Threshold\s+\d+',
            r'^Entry\s+\d+',
            
            # MISSING ENTRY TYPES from original script:
            # Whispered Flames (numbered)
            r'^Whispered\s+Flame\s+\d+',
            r'Whispered\s+Flame\s+\d+[:\-–—]',
            r'Whispered\s+Flame\s+\d+\s*"',
            r'Whispered\s+Flame\s+\d+\s*\*',
            
            # Field Pulses (numbered)
            r'^Field\s+Pulse\s+\d+',
            r'Field\s+Pulse\s+\d+[:\-–—]',
            ...
            # Special cases with emojis
            r'^📓\s*AmandaMap\s+Threshold',  # Emoji headers
    ...
    def extract_threshold_number_from_content(self, content: str) -> Optional[str]:
        ...
        patterns = [
            ...
            # Additional patterns found in actual chats
            r'AmandaMap\s+Threshold\s+(\d+)[:\-–—]',  # Threshold 38:, Threshold 27 –, etc.
            r'AmandaMap\s+Threshold\s+(\d+)\s*\(',     # Threshold 54 (In Progress)
            r'AmandaMap\s+Threshold\s+(\d+)\s*"',      # Threshold 53: "Title"
            r'AmandaMap\s+Threshold\s+(\d+)\s*\*',     # Threshold 28 – *Title*
            ...
            # NEW ENTRY TYPES from original script:
            # Whispered Flames (numbered)
            r'Whispered\s+Flame\s+(\d+)',
            r'Whispered\s+Flame\s+(\d+)[:\-–—]',
            r'Whispered\s+Flame\s+(\d+)\s*"',
