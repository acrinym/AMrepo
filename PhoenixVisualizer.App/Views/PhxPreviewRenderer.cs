...
public class PhxPreviewRenderer
{
    ...
    private readonly PhxCodeEngine _codeEngine;
    ...
    private void RenderFrame(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            // Update code engine time
            _codeEngine.UpdateTime(1.0f / 60.0f); // Assume 60 FPS

            // Execute frame code if available
            if (!string.IsNullOrWhiteSpace(_viewModel.FrameCode))
            {
                var frameResult = _codeEngine.ExecuteFrame(_viewModel.FrameCode);
                if (!frameResult.Success)
                {
                    // Handle frame code errors
                    _viewModel.StatusMessage = $"Frame Error: {frameResult.Message}";
                }
            }
    ...
    private void RenderEffectStack()
    {
        ...
        var context = new RenderContext
        {
            Width = 300,
            Height = 250,
            Time = _audioFeatures.TimeSeconds,
    ...
    private void RenderEffectToBuffer(IEffectNode effect, Dictionary<string, EffectParam> parameters,
    ...
    private uint GenerateEffectColor(IEffectNode effect, Dictionary<string, EffectParam> parameters,
    ...
    private float ApplyEffectModifications(IEffectNode effect, Dictionary<string, EffectParam> parameters,
    ...
    private uint PatternToColor(float pattern, IEffectNode effect, Dictionary<string, EffectParam> parameters)
    {
            ...
            unsafe
            {
                ...
                for (int y = 0; y < 250; y++)
                {
                    for (int x = 0; x < 300; x++)
                    {
                        ...
                        float time = _audioFeatures.TimeSeconds;
    ...
    private void UpdateAudioData()
    {
        ...
        {
            float phase = _audioFeatures.TimeSeconds * 2 + i * 0.1f;
            _waveformBuffer[i] = (float)(Math.Sin(phase) * 0.5 + Math.Sin(phase * 2) * 0.3);
        }
        ...
        for (int i = 0; i < _spectrumBuffer.Length; i++)
        {
            ...
            _spectrumBuffer[i] = (float)(Math.Exp(-freq * 2) * (0.5 + Math.Sin(_audioFeatures.TimeSeconds + freq * 10) * 0.5));
        }
    }
